    ...
    <v-container v-if="!isLoading" class='my-5' style="max-width: 32rem">
        <v-form v-model='isFormValid' ref='form'>
            <v-simple-table dense>
                <template v-slot:top>
                    <v-toolbar flat>
                        <v-toolbar-title>Edit zipCode</v-toolbar-title>
                    </v-toolbar>
                </template>
                <tbody>
                    <tr>
                    ...
                    <tr>
                        <td>Name</td>
                        <td>
                            <v-text-field v-model="zipCodes.name" :rules="[ruleRequired, ruleName]"></v-text-field>
                        </td>
                    </tr>
                </tbody>
            </v-simple-table>
        ...
        <v-row class="pa-5" justify="space-between">
            ...
            <v-btn class="error" @click="deleteZipCode">Delete</v-btn>
...
export default {
  ...
  methods: {
    ...
    deleteZipCode: async function () {
        const response = await fetch(`/api/zipCodes/${this.$route.params.id}`, {
        method: "DELETE",
      });
      if (response.ok) {
        this.$router.push("/admin");
  ...
  mounted() {
      fetch(`/api/zipCodes/${this.$route.params.id}`)
      .then(response => {
        if (!response.ok) {
          throw new Error();
        }
