<template>
    <v-menu
        ...
        <v-date-picker
            :value='returnDate'
            @input='returnDateMenu = false, $emit("update", $event)'
...
module.exports = {
    ...
    methods: {
        allowedReturnDates: function (date) {
            ...
            return this.returnDates?.filter(returnDate => {
                return ((this.departureDate == null || returnDate > new Date(this.departureDate).setHours(0, 0, 0)) && returnDate > new Date().getTime())
