<template>
    <v-menu
        ...
        <v-date-picker
            :value='departureDate'
            @input='departureDateMenu = false, $emit("update", $event)'
...
module.exports = {
    ...
    methods: {
        allowedDepartureDates: function (date) {
            ...
            return this.departureDates?.filter(departureDate => {
                return ((this.returnDate == null || departureDate < new Date(this.returnDate).setHours(0, 0, 0)) && departureDate > new Date().getTime())
